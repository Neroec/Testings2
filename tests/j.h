...
TEST(j_positive, no_next_line_after_next)
{
    ...
    EXPECT_EQ(j(txt), 0);
    EXPECT_EQ(strcmp(nd1->contents, "HelloyHelloy\0"), 0);
    EXPECT_EQ(nd1->next, (node*) NULL);
    EXPECT_EQ(txt->end, nd1);

    free(txt); free(cur); free(nd1);
}
...
TEST(j_positive, line_after_next)
{
    ...
    EXPECT_EQ(j(txt), 0);
    EXPECT_EQ(strcmp(nd1->contents, "HelloyHelloy\n"), 0);
    EXPECT_EQ(nd1->next, nd3);
    EXPECT_EQ(nd3->previous, nd1);

    free(txt); free(cur); free(nd1); free(nd3);
}

TEST(j_negative, last_line)
{
    char line[MAXLINE+1] = {"Helloy\n\0"};
